...
- `build_minimal_world()` deterministically creates campaign-local shared-device fraud sequences plus one campaign-external shared-device lookalike with longer seeded transfer delays.
...
- Python is unavailable on this agent shell's `PATH`, so the suite cannot be executed here. The latest user-run local suite passed 51/51.
