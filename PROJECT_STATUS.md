...
V0 — Agent / Human Approval Loop is **IMPLEMENTED / ACCEPTANCE VERIFIED**. V0.5 — Investigator-Ready Investigation Loop is **IN PROGRESS**. Its approved documentation baseline and Tasks 1–3 are committed and pushed; Task 4 direction-aware proposal-agent/model-boundary work is local and pending review.
...
- `docs/specs/V0_5_INVESTIGATOR_READY_LOOP.md` is the approved V0.5 behavior, scope, and acceptance specification. Its implementation is in progress; Tasks 1–3 are committed and Task 4 direction-aware proposal-agent/model-boundary work is complete locally pending review.
...
- V0.5 Task 4 adds direction-aware, governed proposal generation; explicit local-model configuration; model preflight; and categorized local-model failures. This work awaits review and commit.
- V0.5 lifecycle/orchestration and investigator UI work have not started.
...
No further synthetic-case work is approved. The fixture is frozen. V0.5 Task 4 is pending review; Task 5 requires separate approval after Task 4 is accepted. V1 — Controlled execution has not started and requires separate planning and approval.
