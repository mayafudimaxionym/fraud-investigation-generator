...
V0 — Agent / Human Approval Loop is **IMPLEMENTED / ACCEPTANCE VERIFIED**. V0.5 — Investigator-Ready Investigation Loop is **IN PROGRESS**. Its approved documentation baseline, Task 1 `InvestigationDirection` domain contract, and Task 2 SQLite persistence foundation are committed and pushed; Task 3 safe configured case-catalog work is local and pending review.
...
- `docs/specs/V0_5_INVESTIGATOR_READY_LOOP.md` is the approved V0.5 behavior, scope, and acceptance specification. Its implementation is in progress; Tasks 1–2 are committed and Task 3 safe configured case-catalog work is complete locally pending review.
...
- V0.5 Task 3 adds a fixed configured case catalog that resolves opaque package associations only through the governed package loader, plus explicit legacy association persistence. This work awaits review and commit.
- V0.5 proposal-agent, lifecycle, orchestration, and investigator UI work have not started.
...
No further synthetic-case work is approved. The fixture is frozen. V0.5 Task 3 is pending review; Task 4 requires separate approval after Task 3 is accepted. V1 — Controlled execution has not started and requires separate planning and approval.
